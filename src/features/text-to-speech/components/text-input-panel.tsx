...
import { ttsFormOptions, type TTSFormValues } from "./text-to-speech-form";
import { GenerateButton } from "./generate-button";
import { PromptSuggestions } from "./prompt-suggestions";

type TextFieldRenderProps = {
  state: { value: TTSFormValues["text"] };
  handleChange: (value: TTSFormValues["text"]) => void;
};
...
export function TextInputPanel() {
  ...
  const text = useSelector(
    form.store,
    (state: AnyFormState) => state.values.text,
  );
  ...
  const isValid = useSelector(
    form.store,
    (state: AnyFormState) => state.isValid,
  );
